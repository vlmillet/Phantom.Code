...
int main(int argc, char** argv)
{
    ...
    Main app(main, "HelloWorld.Code", argc, argv);

    auto sessionId = Compiler::Get()->newSession();

    auto compiledSource = Compiler::Get()->buildSource(
    sessionId, phantom::Path(__FILE__).parentPath().childPath("HelloWorld.Code.cpplite").genericString());
